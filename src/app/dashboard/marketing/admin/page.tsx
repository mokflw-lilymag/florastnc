    ...
    <div className="container mx-auto p-8 max-w-6xl space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <h1 className="text-4xl font-black tracking-tight">플랫폼 홍보 마스터 센터</h1>
          </div>
          <p className="text-muted-foreground">전체 화원사들의 AI 홍보 엔진을 구동하기 위한 글로벌 API 및 시스템 설정을 관리합니다.</p>
        </div>
      </div>
      ...
      <Tabs defaultValue="sns" className="w-full">
        <TabsList className="grid w-full max-w-2xl grid-cols-4 mb-8 h-12 bg-slate-50 dark:bg-slate-900 ring-1 ring-slate-100 dark:ring-slate-800">
          <TabsTrigger value="sns" className="gap-2 font-bold"><Globe className="w-4 h-4" /> SNS 연동</TabsTrigger>
          <TabsTrigger value="ai" className="gap-2 font-bold"><Brain className="w-4 h-4" /> AI 엔진</TabsTrigger>
          <TabsTrigger value="workflow" className="gap-2 font-bold"><Webhook className="w-4 h-4" /> 워크플로우</TabsTrigger>
          <TabsTrigger value="global" className="gap-2 font-bold"><Zap className="w-4 h-4" /> 공통 제약</TabsTrigger>
        </TabsList>
          ...
          <Card className="border-none shadow-xl ring-1 ring-slate-100 dark:ring-slate-800">
            <CardHeader className="border-b bg-slate-50/50 dark:bg-slate-900/50">
              <CardTitle className="flex items-center gap-2">
                <Brain className="w-5 h-5 text-purple-600" /> AI 모델 전역 키 설정
              </CardTitle>
              <CardDescription>홍보 엔진 구동에 사용되는 멀티 모델(OpenAI, Anthropic 등)의 마스터 키입니다.</CardDescription>
            </CardHeader>
            <CardContent className="pt-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  ...
                  />
                  <Button size="sm" onClick={() => handleSave('openai_key', config.openai_key)}>저장하기</Button>
                </div>
                <div className="space-y-4">
                  ...
                  />
                  <Button size="sm" onClick={() => handleSave('anthropic_key', config.anthropic_key)}>저장하기</Button>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="workflow">
          <Card className="border-none shadow-xl ring-1 ring-slate-100 dark:ring-slate-800">
            <CardHeader className="border-b bg-slate-50/50 dark:bg-slate-900/50">
              <CardTitle className="flex items-center gap-2">
                <Webhook className="w-5 h-5 text-indigo-600" /> n8n 마스터 파이프라인
              </CardTitle>
              <CardDescription>모든 캠페인 작전이 사출되는 n8n 통합 웹훅 주소입니다.</CardDescription>
            </CardHeader>
            <CardContent className="pt-8 space-y-6">
              <div className="grid grid-cols-1 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <Button onClick={() => handleSave('n8n_master_url', config.n8n_master_url)} className="bg-indigo-600">연결 저장</Button>
                  </div>
                </div>
              </div>
            </CardContent>
    ...
    <Card className="border-none shadow-lg ring-1 ring-slate-100 dark:ring-slate-800 overflow-hidden">
      ...
      <CardContent className="p-6 space-y-4">
        ...
        <Button size="sm" variant="secondary" className="w-full font-bold h-9" onClick={() => onSave(localData)}>
          <Save className="w-3.5 h-3.5 mr-2" /> 설정 업데이트
        </Button>
      </CardContent>
    </Card>
  );
