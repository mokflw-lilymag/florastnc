...
export default function ExternalOrdersPage() {
    ...
    const [activeAlbumTab, setActiveAlbumTab] = useState("전체");
    ...
    const categories = ['전체', '꽃다발', '꽃바구니', '축하화환', '근조화환', '동양란/서양란', '식물/분재'];

    const filteredPartners = partners.filter(p => {
        const matchRegion = !filterRegion || p.partner_region?.includes(filterRegion);
        const matchCategory = filterCategory === "전체" || p.partner_category?.includes(filterCategory);
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="space-y-4">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black tracking-tight leading-tight">
                            전국 우수 파트너 <br/>
                            <span className="bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent">프리미엄 네트워크</span>
                        </h1>
                        <p className="text-slate-400 text-sm md:text-base font-light max-w-xl">
                            검증된 파트너들의 포트폴리오를 탐색하고 협력의 기회를 찾으세요. <br/>
                            이미지를 클릭하면 실시간 제작 앨범과 가격 정보를 확인할 수 있습니다.
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        <Link href="/dashboard/external-orders/received">
                            <Button variant="outline" className="h-14 px-8 rounded-2xl border-white/10 bg-white/5 hover:bg-white/10 text-white font-bold backdrop-blur-md">
                                <Package className="mr-3 w-5 h-5 opacity-60" /> 수주 내역 관리
                            </Button>
                        </Link>
                        <Button 
                            className={cn(
                        ...
                        >
                            {myTenant?.can_receive_orders ? <><Check className="mr-3 w-5 h-5" /> 나의 매장 수주 중</> : "나의 매장 수주사로 등록하기"}
                        </Button>
                        ...
                        {myTenant?.can_receive_orders && (
                            <Dialog>
                                <DialogTrigger render={
                                    <Button variant="outline" className="h-14 px-8 rounded-2xl border-white/10 bg-white/5 hover:bg-white/10 text-white font-bold backdrop-blur-md">
                                        <Settings className="mr-3 w-5 h-5 opacity-60" /> 프로필 및 앨범 설정
                                    </Button>
                                } />
                                <DialogContent className="max-w-2xl rounded-[2.5rem] border-0 shadow-3xl p-0 overflow-hidden bg-white">
                                    <div className="bg-slate-900 p-8 text-white">
                                        <h3 className="text-xl font-bold">나의 수주사 프로필 및 품목별 앨범 설정</h3>
                                        <p className="text-slate-400 text-xs mt-1">발주사들이 품목별로 앨범을 볼 수 있도록 각 폴더 ID를 등록하세요.</p>
                                    </div>
                                    <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">대표 앨범 (전체보기용)</label>
                                            <Input id="portfolio-id" placeholder="Google Drive 폴더 ID 또는 URL" defaultValue={myTenant?.portfolio_gdrive_id || ''} className="rounded-xl h-12 bg-slate-50 border-0 focus:ring-2 focus:ring-indigo-500" />
                                        </div>

                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                                            <div className="space-y-2">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">꽃다발 폴더 ID</label>
                                                <Input id="bouquet-id" placeholder="폴더 ID" defaultValue={myTenant?.gdrive_bouquet_id || ''} className="rounded-lg h-10 bg-white border-slate-200 text-xs" />
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">꽃바구니 폴더 ID</label>
                                                <Input id="basket-id" placeholder="폴더 ID" defaultValue={myTenant?.gdrive_basket_id || ''} className="rounded-lg h-10 bg-white border-slate-200 text-xs" />
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">축하화환 폴더 ID</label>
                                                <Input id="wreath-id" placeholder="폴더 ID" defaultValue={myTenant?.gdrive_wreath_id || ''} className="rounded-lg h-10 bg-white border-slate-200 text-xs" />
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">근조화환 폴더 ID</label>
                                                <Input id="condolence-id" placeholder="폴더 ID" defaultValue={myTenant?.gdrive_condolence_id || ''} className="rounded-lg h-10 bg-white border-slate-200 text-xs" />
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">동양란/서양란 폴더 ID</label>
                                                <Input id="orchid-id" placeholder="폴더 ID" defaultValue={myTenant?.gdrive_orchid_id || ''} className="rounded-lg h-10 bg-white border-slate-200 text-xs" />
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">식물/분재 폴더 ID</label>
                                                <Input id="plant-id" placeholder="폴더 ID" defaultValue={myTenant?.gdrive_plant_id || ''} className="rounded-lg h-10 bg-white border-slate-200 text-xs" />
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div className="space-y-3">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1 flex items-center gap-2 animate-in fade-in slide-in-from-left duration-500">
                                                    <div className="w-1 h-1 bg-indigo-500 rounded-full" /> 품질/전문성 한줄 홍보
                                                </label>
                                                <Input id="partner-desc" placeholder="예: 25년 장인의 프리미엄 화환 전문점" defaultValue={myTenant?.partner_description || ''} className="rounded-xl h-12 bg-slate-50 border-0 focus:ring-2 focus:ring-indigo-500 shadow-inner" />
                                            </div>
                                            <div className="space-y-3">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1 flex items-center gap-2 animate-in fade-in slide-in-from-left duration-700">
                                                    <div className="w-1 h-1 bg-indigo-500 rounded-full" /> 연락처 (발주사 확인용)
                                                </label>
                                                <Input id="contact-phone" placeholder="예: 010-1234-5678" defaultValue={myTenant?.contact_phone || ''} className="rounded-xl h-12 bg-slate-50 border-0 focus:ring-2 focus:ring-indigo-500 shadow-inner" />
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl">
                                             ...
                                             <div className="relative z-10 space-y-4">
                                                 <div className="flex items-center gap-3">
                                                     ...
                                                     <h4 className="font-black text-lg">앨범 구성 및 매출 극대화 가이드</h4>
                                                 </div>
                                                 <div className="grid grid-cols-1 gap-3">
                                                     <div className="p-4 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-4 group hover:bg-white/10 transition-all">
                                                         ...
                                                         <p className="text-[11px] font-light leading-relaxed"><span className="text-indigo-400 font-black">파일명 형식 준수:</span> 파일명을 <span className="bg-white/10 px-2 py-0.5 rounded font-bold text-indigo-300">[상품명] 가격.jpg</span> 으로 지정하면 발주사 신뢰도가 급상승합니다.</p>
                                                     </div>
                                                     <div className="p-4 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-4 group hover:bg-white/10 transition-all">
                                                         ...
                                                         <p className="text-[11px] font-light leading-relaxed"><span className="text-indigo-400 font-black">카테고리 분리:</span> 전체 폴더 하나보다 품목별(꽃다발, 화환 등)로 폴더를 분리하여 등록하는 것이 <span className="text-amber-400 font-bold">노출 확률 200% 상승</span> 비결입니다.</p>
                                                     </div>
                                                 </div>
                                             </div>
                                        </div>
                                        ...
                                        <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-start gap-3">
                                            ...
                                            <p className="text-[11px] text-indigo-700 leading-relaxed">
                                                각 카테고리별 폴더 ID를 입력하면 발주사가 해당 카테고리를 클릭했을 때 해당 폴더가 직접 열립니다. 입력하지 않으면 대표 앨범이 표시됩니다.
                                            </p>
                                        </div>
            ...
            <Dialog open={showTerms} onOpenChange={setShowTerms}>
                <DialogContent className="max-w-xl rounded-[3rem] p-0 overflow-hidden border-0 shadow-3xl bg-white">
                    <div className="bg-indigo-600 p-10 text-white">
                        ...
                        <h3 className="text-3xl font-black leading-tight">전국 협력 네트워크 <br/>참여 약관 서약</h3>
                    </div>
                    <div className="p-10 space-y-6 max-h-[50vh] overflow-y-auto bg-slate-50/50">
                        {[
                            { title: "정산 시스템 준수", desc: "수주사 79% / 발주사 19% / 플랫폼 2% 정산 기준을 준수합니다." },
                            { title: "정산 주기 확인", desc: "클레임 대비를 위해 배송 완료 후 일정 기간 이후 출금 가능함을 이해합니다." },
                            { title: "제작 사진 의무화", desc: "모든 주문은 실제 제작 사진과 배송 완료 사진을 실시간으로 업로드해야 합니다." }
                        ].map((item, idx) => (
                    ...
                    <div className="p-10 bg-white flex gap-4">
                        <Button variant="ghost" className="flex-1 h-14 rounded-2xl font-bold text-slate-400" onClick={() => setShowTerms(false)}>취소</Button>
                        <Button className="flex-[2] h-14 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold shadow-xl shadow-indigo-100" onClick={() => { setShowTerms(false); updateMyPartnerStatus({ can_receive_orders: true }); }}>약관 동의 및 수주 등록</Button>
                    </div>
                </DialogContent>
            </Dialog>
            ...
            <div className="space-y-8">
                ...
                {premiumPartners.length === 0 ? (
                    <div className="h-64 rounded-[3rem] border border-dashed border-slate-200 bg-slate-50/50 flex flex-col items-center justify-center text-slate-300">
                        ...
                        <p className="font-bold">조건에 맞는 프리미엄 파트너를 준비 중입니다.</p>
                    </div>
                ) : (
            ...
            <div className="space-y-8">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-slate-800">일반 협력사 디렉토리</h3>
                            <p className="text-xs text-slate-400 font-medium">{generalPartners.length}개의 매장이 활동 중입니다.</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto scrollbar-hide">
                        <table className="w-full text-sm text-left border-collapse">
                            <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                                <tr>
                                    <th className="px-10 py-6">매장 정보</th>
                                    <th className="px-10 py-6">지역 / 카테고리</th>
                                    <th className="px-10 py-6 text-center">리얼 앨범(쇼핑몰)</th>
                                </tr>
                            </thead>
            ...
            <div className="p-8 pb-4 flex-1 space-y-6">
                    ...
                    <div className="flex flex-col items-end gap-1.5 pt-2">
                        ...
                        <div className="flex flex-col items-end gap-1 text-[10px] text-slate-400 font-bold">
                            <div className="flex items-center gap-1">
                                <MapPin size={10} /> {partner.partner_region || '전국'}
                            </div>
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-slate-500 font-light leading-relaxed line-clamp-3 h-[3.3rem] opacity-70 italic">
                        {partner.partner_description || '아직 등록된 매장 소개가 없습니다.'}
                    </p>
                </div>

                <div className="flex flex-wrap gap-1.5">
                    {partner.partner_category?.split(',').map((c: string) => (
                        <Badge key={c} variant="outline" className="text-[9px] font-bold text-indigo-500 border-indigo-100 bg-indigo-50/50 rounded-full">{c}</Badge>
                    ))}
                    {!partner.partner_category && <Badge variant="outline" className="text-[9px] font-bold text-slate-400 border-slate-100 rounded-full">일반파트너</Badge>}
                </div>
            </div>
            ...
            <div className="p-8 pt-0 mt-auto">
                <div className="flex items-center gap-3">
                    {partner.portfolio_gdrive_id ? (
                        ...
                        <div className="flex-1 h-14 flex items-center justify-center text-[10px] text-slate-300 italic bg-slate-50 rounded-2xl border border-slate-100">앨범 준비 중</div>
                    )}
                    <Button 
                        variant="outline" 
            ...
            <td className="px-10 py-8">
                <div className="flex items-center gap-5">
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-3">
                            <p className="text-[11px] text-slate-400 font-light line-clamp-1 max-w-[200px] italic">{partner.partner_description || '소개 내용이 없습니다.'}</p>
            ...
            <td className="px-10 py-8">
                <div className="flex items-center gap-3">
                    <Badge variant="outline" className="text-[10px] font-bold text-slate-500 border-slate-200 bg-white rounded-full px-4 py-1.5 flex items-center gap-2">
                        <MapPin size={10} className="text-slate-300" /> {partner.partner_region || '전국'}
                    </Badge>
            ...
            <td className="px-10 py-8">
                <div className="flex items-center gap-2 justify-center">
                    {partner.portfolio_gdrive_id ? (
                        ...
                        <span className="text-[10px] text-slate-300 italic font-light">앨범 미등록</span>
                    )}
                    <Button 
                        variant="ghost" 
                        size="sm" 
            ...
            <DialogTrigger render={
                variant === "full" ? (
                    <Button className="w-full h-14 bg-slate-900 hover:bg-indigo-600 text-white rounded-2xl font-bold transition-all shadow-xl group-hover:shadow-indigo-600/20">
                        <Store className="mr-3 w-5 h-5 opacity-70" /> 앨범(쇼핑몰) 입장
                    </Button>
                ) : (
                    ...
                    <div className="flex items-center gap-6 relative z-10">
                        ...
                        <div>
                            <div className="flex items-center gap-3 mb-1">
                                ...
                                <span className="text-white/40 text-[10px] font-bold flex items-center gap-1"><MapPin size={10} /> {partner.partner_region}</span>
                            </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 md:p-12 space-y-12">
                     <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                        <div className="space-y-4">
                            <h4 className="text-2xl font-black text-slate-800 flex items-center gap-3">
                                <LayoutGrid className="text-indigo-600" /> 포트폴리오 탐색
                            </h4>
                        ...
                        <div className="flex items-center gap-3 bg-white px-6 py-4 rounded-[2rem] border border-slate-100 shadow-sm animate-in slide-in-from-bottom duration-700">
                             ...
                             <div>
                                ...
                                <p className="text-[10px] text-slate-400 font-light">작업 사진 파일명에 <span className="font-bold text-indigo-600">[상품명] 가격</span>이 표기되어 있습니다.</p>
                             </div>
                        </div>
                    </div>
                    ...
                    <div className="relative group/gallery">
                        <div className="h-[650px] md:h-[calc(95vh-400px)] min-h-[500px] rounded-[4rem] overflow-hidden border-[12px] border-white shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] bg-white relative transition-all duration-500 group-hover/gallery:border-indigo-50">
                            ...
                            <div className="absolute bottom-6 right-6 flex items-center gap-2">
                                ...
                                >
                                    <ExternalLink size={14} /> 새 창에서 폴더 확인 (공유 권한 체크)
                                </Button>
                            </div>
                            ...
                            {activeTab !== "전체" && (
                                <div className="absolute top-10 right-10 animate-in slide-in-from-right fade-in duration-700 pointer-events-none">
                                    <div className="flex items-center gap-3 bg-indigo-600/90 backdrop-blur-xl text-white px-8 py-4 rounded-full shadow-2xl border border-white/20">
                                        ...
                                        <span className="text-[13px] font-black tracking-tight">{activeTab} 섹션 탐색 중...</span>
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="p-12 bg-white rounded-[4rem] border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-12">
                         <div className="space-y-4 max-w-xl">
                            <h5 className="text-xl font-black text-slate-800 italic">"진심을 담은 꽃, 최상의 정품 퀄리티를 약속드립니다."</h5>
                            <p className="text-sm text-slate-400 leading-relaxed font-light">
                                본 앨범의 모든 사진은 {partner.name}에서 직접 제작한 실제 작업 결과물입니다. 
                                화환 및 꽃 상품 주문 시 앨범의 느낌과 동일한 신선한 컨디션으로 제작 및 안전 배송됩니다.
                            </p>
                         </div>
                    </div>
                </div>
