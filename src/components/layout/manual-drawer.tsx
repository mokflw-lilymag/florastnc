...
export function ManualDrawer() {
  ...
  const sections = [
    {
      id: "step1-settings",
      title: tr("STEP 1. 환경설정 & 화원 정보 입력", "STEP 1. Configure settings & store details"),
        ...
        <div className="space-y-4">
          <p className="text-sm text-slate-600 font-medium leading-relaxed">
            {tr(
              ...
              "Before operations begin, set your store information as the system baseline."
            )}
          </p>
          <div className="p-5 bg-white border border-slate-100 rounded-3xl shadow-sm space-y-4">
             <div className="flex items-start gap-3">
                ...
                <div>
                   <p className="text-xs font-black text-slate-800">{tr("[환경설정 > 매장 정보]", "[Settings > Store Info]")}</p>
                   <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">
                      {tr(
                        "상호명, 대표자, 사업자 번호, 연락처를 입력하세요. 여기서 입력한 정보가 **주문서 하단** 및 **정량화된 영수증**에 자동으로 반영됩니다.",
                        "Enter your business name, owner, registration number, and contact details. This information is automatically shown in the order footer and itemized receipts."
                      )}
                   </p>
                </div>
             </div>
             <div className="flex items-start gap-3">
                ...
                <div>
                   <p className="text-xs font-black text-slate-800">{tr("프린터 & 하드웨어 연결", "Printer & hardware setup")}</p>
                   <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">
                      {tr(
                        "리본 프린터 기종을 선택하고 '연결 테스트'를 진행하세요. ",
                        "Select your ribbon printer model and run a connection test. "
                      )}
                      <Badge variant="outline" className="text-[9px] h-4">v25.0 Bridge</Badge>
                      {tr("가 설치되어 있어야 합니다.", " must be installed.")}
                   </p>
                </div>
             </div>
          </div>
    ...
    {
      id: "step2-categories",
      title: tr("STEP 2. 카테고리 & 과제(관리 항목) 설정", "STEP 2. Configure categories & tasks"),
      icon: <Layers className="w-6 h-6 text-emerald-500" />,
      content: (
        <div className="space-y-4">
          <p className="text-sm text-slate-600 font-medium italic">
            {tr("\"복잡한 장부를 깔끔하게 나누는 기준이 됩니다.\"", "\"This becomes the standard for organizing complex bookkeeping cleanly.\"")}
          </p>
          <div className="p-6 bg-emerald-50/50 border border-emerald-100 rounded-3xl space-y-4">
             <div className="space-y-2">
                <p className="text-xs font-black text-emerald-700">{tr("📌 과제(Task) 관리란?", "📌 What is Task management?")}</p>
                <p className="text-[11px] text-emerald-800/70 leading-relaxed font-semibold">
                   {tr(
                     "본점, 분점 혹은 품목별(꽃/난/부자재)로 분류를 나누는 기능입니다. 이렇게 나눠두면 연말 정산 시 **\"꽃에서만 얼마를 벌었는지\"** 정확한 통계가 나옵니다.",
                     "This lets you split records by HQ, branch, or item types (flowers/orchids/materials). With this setup, year-end settlement can show precise stats like \"how much profit came from flowers only\"."
                   )}
                </p>
             </div>
             <div className="grid grid-cols-2 gap-3">
                <div className="p-4 bg-white rounded-2xl border border-emerald-100">
                   ...
                   <p className="text-xs font-bold text-slate-800">{tr("카테고리 수정", "Edit categories")}</p>
                   <p className="text-[10px] text-slate-500 mt-1">{tr("개업, 축하, 근조 등 분류 생성", "Create groups such as opening, celebration, condolence")}</p>
                </div>
                <div className="p-4 bg-white rounded-2xl border border-emerald-100">
                   <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Action</p>
                   <p className="text-xs font-bold text-slate-800">{tr("과제(지출) 설정", "Set tasks (expenses)")}</p>
                   <p className="text-[10px] text-slate-500 mt-1">{tr("임대료, 인건비 등 지출 항목", "Expense items such as rent and labor cost")}</p>
                </div>
             </div>
          </div>
        </div>
    ...
    },
    {
      ...
      title: tr("STEP 3. 기초 자료 일괄 등록 (엑셀)", "STEP 3. Bulk upload master data (Excel)"),
             ...
             <div className="relative z-10 flex flex-col md:flex-row gap-6 items-center">
                ...
                <div className="flex-1 space-y-2">
                   ...
                   <p className="text-xs text-slate-400 font-medium">
                      {tr("일일이 하나씩 등록할 필요 없습니다.", "No need to register items one by one.")}
                      <br />
                      {tr("기존에 쓰시던 엑셀 자료를 단 5초 만에 시스템에 밀어넣으세요.", "Upload your existing Excel data into the system in seconds.")}
                   </p>
                </div>
             </div>
                ...
                <table className="w-full text-left">
                   <thead>
                      <tr className="bg-white/10 border-b border-white/5">
                         <th className="p-3 text-[10px] font-black text-slate-400">{tr("등록 항목", "Upload item")}</th>
                         <th className="p-3 text-[10px] font-black text-slate-400">{tr("방법", "Method")}</th>
                      </tr>
                   </thead>
                   <tbody className="text-[11px]">
                      <tr className="border-b border-white/5">
                         <td className="p-3 font-bold text-amber-400 whitespace-nowrap">{tr("상품 마스터 (Product)", "Product master")}</td>
                         <td className="p-3 text-slate-300 font-medium">{tr("템플릿 다운로드 ➔ 상품명/원가 작성 ➔ 업로드", "Download template -> fill product name/cost -> upload")}</td>
                      </tr>
                      <tr>
                         <td className="p-3 font-bold text-blue-400 whitespace-nowrap">{tr("거래처 마스터 (Account)", "Account master")}</td>
                         <td className="p-3 text-slate-300 font-medium">{tr("단골 퀵 업체, 거래 농장 리스트 일괄 등록", "Bulk upload preferred delivery vendors and supplier farms")}</td>
                      </tr>
                   </tbody>
                </table>
             </div>
    ...
    {
      id: "step4-daily-ops",
      title: tr("STEP 4. 실전! 주문 접수 및 배송/정산", "STEP 4. Live ops: orders, delivery, and settlement"),
        ...
        <div className="space-y-4">
           <div className="flex flex-col md:flex-row gap-4 items-stretch">
             <div className="flex-1 p-5 rounded-2xl bg-slate-50 border border-slate-100 flex flex-col justify-between">
                <p className="text-xs font-black text-slate-400 mb-1 tracking-widest uppercase">{tr("01. 주문", "01. Orders")}</p>
                <p className="text-sm font-bold text-slate-800">{tr("번호 4자리로 고객 조회", "Find customer by 4-digit number")}</p>
                <p className="text-[11px] text-slate-500 mt-2 leading-relaxed">{tr("상담과 동시에 화면에 정보를 띄워 응대하세요.", "Show customer info instantly while taking the call.")}</p>
             </div>
             <div className="flex-1 p-5 rounded-2xl bg-indigo-50 border border-indigo-100 flex flex-col justify-between shadow-sm">
                <p className="text-xs font-black text-indigo-400 mb-1 tracking-widest uppercase">{tr("02. 출력", "02. Print")}</p>
                <p className="text-sm font-bold text-indigo-700">{tr("메시지 즉시 출력", "Print message instantly")}</p>
                <p className="text-[11px] text-indigo-600/70 mt-2 leading-relaxed">{tr("프린터 아이콘 한 번으로 퀄리티 높은 리본이 출력됩니다.", "One tap on the printer icon outputs a high-quality ribbon.")}</p>
             </div>
             <div className="flex-1 p-5 rounded-2xl bg-amber-50 border border-amber-100 flex flex-col justify-between">
                <p className="text-xs font-black text-amber-400 mb-1 tracking-widest uppercase">{tr("03. 정산", "03. Settlement")}</p>
                <p className="text-sm font-bold text-amber-700">{tr("실 배송비 입력", "Enter actual delivery cost")}</p>
                <p className="text-[11px] text-amber-600/70 mt-2 leading-relaxed">{tr("퀵 비용을 입력하면 장부에 자동 기록됩니다.", "Quick-delivery fees are automatically logged in the ledger.")}</p>
             </div>
          </div>
        </div>
      )
    ...
    <Dialog>
      <DialogTrigger className="relative group flex max-w-full min-w-0 shrink items-center gap-2 rounded-full border-2 border-indigo-200 bg-white px-3 py-2 font-bold text-indigo-700 shadow-lg shadow-indigo-100/50 outline-none cursor-pointer transition-all hover:border-indigo-500 hover:bg-indigo-50 md:h-11 md:gap-3 md:px-5">
          ...
          <span className="hidden min-w-0 truncate text-xs sm:inline sm:text-sm md:max-w-[11rem] lg:max-w-none">
            {tr("플록싱크 완벽 세팅 가이드", "Floxync complete setup guide")}
          </span>
          <span className="text-xs sm:hidden">{tr("가이드", "Guide")}</span>
          ...
          <DialogHeader className="p-10 bg-slate-950 text-white shrink-0 relative overflow-hidden">
             ...
             <div className="relative z-10">
                ...
                <DialogDescription className="text-slate-400 font-medium text-base leading-relaxed max-w-2xl">
                   {tr("사장님, 환경설정부터 기초 자료 업로드까지 순서대로 따라오시면", "From settings to master-data upload, just follow each step in order")}
                   <br />
                   {tr("복잡했던 화원 운영이 마법처럼 시스템화됩니다.", "and your complex flower-shop operations become systemized like magic.")}
                </DialogDescription>
             </div>
             ...
             <div className="mt-8 relative z-10">
                ...
                <Input 
                   ...
                   placeholder={tr("어느 단계가 궁금하신가요? (예: 과제 설정, 매장 정보)", "Which step do you need help with? (ex: task setup, store info)")} 
            ...
            <div className="space-y-16 pb-12">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 bg-slate-50 p-8 rounded-[3rem] border border-slate-100">
                   <div className="flex flex-col items-center justify-center p-6 bg-white rounded-3xl group cursor-pointer hover:shadow-xl hover:shadow-indigo-500/10 transition-all border border-slate-100">
                      ...
                      <p className="font-bold text-xs">{tr("화원 정보 설정", "Store information setup")}</p>
                   </div>
                   <div className="flex flex-col items-center justify-center p-6 bg-white rounded-3xl group cursor-pointer hover:shadow-xl hover:shadow-emerald-500/10 transition-all border border-slate-100">
                      ...
                      <p className="font-bold text-xs">{tr("과제/분류 마스터", "Task/category master")}</p>
                   </div>
                   <div className="flex flex-col items-center justify-center p-6 bg-white rounded-3xl group cursor-pointer hover:shadow-xl hover:shadow-amber-500/10 transition-all border border-slate-100">
                      ...
                      <p className="font-bold text-xs">{tr("엑셀 대량 업로드", "Bulk Excel upload")}</p>
                   </div>
               </div>
            </div>
          </div>
