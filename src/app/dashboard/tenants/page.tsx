...
export default function TenantsPage() {
  ...
  const getStatusBadge = (tenant: TenantWithProfile) => {
    ...
    if (tenant.status === 'suspended') return <Badge variant="outline" className="bg-red-50 text-red-600 border-red-100 font-normal">{tf.f01823}</Badge>;
    if (isExpired) return <Badge variant="outline" className="bg-amber-50 text-amber-600 border-amber-100 font-normal">{tf.f01143}</Badge>;
    if (!tenant.subscription_end) return <Badge variant="outline" className="bg-blue-50 text-blue-600 border-blue-100 font-normal">{tf.f01208}</Badge>;
    return <Badge variant="outline" className="bg-emerald-50 text-emerald-600 border-emerald-100 font-normal">{tf.f01819}</Badge>;
  };
             ...
             <div className="relative w-full md:w-96">
              ...
              <div className="flex gap-3">
                <Button variant="outline" className="h-12 px-5 rounded-2xl border-slate-100 font-normal bg-white hover:bg-slate-50 text-slate-700">
                  <Filter className="h-4 w-4 mr-2 text-slate-400" /> {tf.f01343}
                </Button>
            ...
            <Table>
              <TableHeader className="bg-slate-50/30">
                <TableRow className="hover:bg-transparent border-slate-50 border-b-0">
                  <TableHead className="px-8 font-medium text-slate-600 py-5 uppercase tracking-wider text-[11px]">{tf.f02213}</TableHead>
                  ...
                  <TableHead className="font-medium text-slate-600 py-5 uppercase tracking-wider text-[11px]">{tf.f00976}</TableHead>
                  <TableHead className="font-medium text-slate-600 py-5 uppercase tracking-wider text-[11px]">{tf.f00940}</TableHead>
                  ...
                  <TableRow key={tenant.id} className="group hover:bg-slate-50/50 transition-colors border-slate-50 border-0">
                    ...
                    <TableCell className="border-0">
                       <div className="flex items-center gap-2">
                         ...
                         <span className="text-slate-700 font-normal">{tenant.profiles?.find(p => p.role === 'tenant_admin')?.email || tenant.profiles?.[0]?.email || tf.f00224}</span>
                       </div>
                    </TableCell>
                    <TableCell className="border-0">
            ...
            <div className="grid gap-2">
              ...
              <Select value={editPlan} onValueChange={(val) => setEditPlan(val || 'free')}>
                ...
                <SelectContent className="rounded-2xl border-0 shadow-xl bg-white">
                  <SelectItem value="free" className="font-normal text-slate-900">{tf.f02282}</SelectItem>
                  <SelectItem value="erp_only" className="font-normal text-slate-900">{tf.f02260}</SelectItem>
                  <SelectItem value="pro" className="font-normal text-slate-900">{tf.f02283}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="grid gap-3">
               ...
               <div className="grid grid-cols-2 gap-2">
                  ...
                  <Button variant="outline" className="h-10 rounded-xl font-normal text-xs justify-start px-3 text-slate-700 border-slate-200 border" onClick={() => setDuration(3)}>
                    {tf.f00839} ({PRICING[editPlan as keyof typeof PRICING]?.["3m"]?.toLocaleString()}{tf.f00487})
                  </Button>
                  <Button variant="outline" className="h-10 rounded-xl font-normal text-xs justify-start px-3 text-slate-700 border-slate-200 border" onClick={() => setDuration(6)}>
                    {tf.f00841} ({PRICING[editPlan as keyof typeof PRICING]?.["6m"]?.toLocaleString()}{tf.f00487})
                  </Button>
