...
import {
  errAdminForbidden,
  errAdminSeedBulkApplyFailedGeneric,
  errAdminSeedBulkTenantLimit,
  errAdminSeedConfirmRequired,
...
export async function POST(req: Request) {
  ...
  if (!confirm) {
    return NextResponse.json({ error: errAdminSeedConfirmRequired(bl) }, { status: 400 });
  }
  if (!versionId?.trim()) {
  ...
  try {
    ...
    console.error("tenant-master-seed apply-bulk", e);
    return NextResponse.json({ error: errAdminSeedBulkApplyFailedGeneric(bl) }, { status: 500 });
  }
}
