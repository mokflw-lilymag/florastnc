...
2. **주문 가져오기 스케줄러 (Cron Jobs / Webhooks)**:
    - **Webhooks**: 플랫폼에서 지원할 경우, 신규 주문 시 FloraSync 서버 알림 엔드포인트 `/api/webhooks/orders`로 즉각 전송받음.
    - **Polling**: 지원하지 않는 플랫폼의 경우 10~15분 주기로 `/api/sync/orders`가 "새로 들어온 주문 가져와!" 요청.
3. **데이터 표정화 (Normalization)**: 카페24의 주문 JSON과 네이버의 주문 JSON은 구조가 다름. 이를 FloraSync 공통 스키마(`pos_orders`, `deliveries`)로 규격 변환하여 삽입.
...
4. FloraSync의 빈칸에 키를 붙여넣고 **[연결 테스트 후 저장]** 완료.
