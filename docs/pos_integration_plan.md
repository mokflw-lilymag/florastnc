# 🏪 Floxync × POS 연동 전략 기획서 (v1.0)

> **작성일**: 2026-04-17  
> **기획**: 총괄 디렉터 에이전트 + 구조 설계 에이전트 + 결제/보안 에이전트 공동 기획  
> **목표**: Floxync SaaS를 독립 운영뿐 아니라, 오프라인 POS 시스템과 완전 연동하여 이중 입력 없는 통합 꽃집 운영 플랫폼 구축
...
### 현재 상황 (As-Is)
Floxync는 현재 웹 기반 독립 SaaS로 동작하며, 매장 사장님이 직접 주문/결제/배송을 수기 입력하는 방식입니다.

### 목표 상황 (To-Be)
오프라인 POS 단말기(이지체크, 토스POS 등)에서 결제가 완료되는 순간, Floxync에 **자동으로** 다음 정보가 동기화됩니다:
- 결제 금액 및 방법
- 구매 상품 정보
- 고객 포인트 자동 적립/차감
...
[POS Bridge Service]  ← 핵심 비즈니스 로직
    ...
    |── 지출 자동 기록
    ↓
[Supabase DB]  →  [Realtime 구독]  →  [Floxync 대시보드 즉시 반영]
```
...
POS 결제 완료 이벤트로 Floxync 주문이 자동 생성될 때 기본값:
...
4. **결제 취소 동기화**: POS에서 결제 취소 시 Floxync 주문도 자동으로 취소 처리할까요?
