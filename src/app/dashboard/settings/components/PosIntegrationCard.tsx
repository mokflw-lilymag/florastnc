      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <Label className="text-sm font-bold flex items-center gap-2">
            <CreditCard className="h-4 w-4 text-orange-600" /> 오프라인 POS 연동 (이지체크/토스)
          </Label>
          <p className="text-xs text-slate-500">포스기에서 결제 시 자동으로 주문을 생성하고 포인트를 적립합니다.</p>
        </div>
        <Link href="/dashboard/settings/pos">
          <Button size="sm" variant="outline" className="gap-2 rounded-xl border-orange-200 text-orange-600 hover:bg-orange-50">
            <SettingsIcon className="h-3 w-3" /> 상세 설정
          </Button>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {isLoading ? (
          <div className="col-span-2 h-24 border border-dashed rounded-2xl flex items-center justify-center bg-slate-50/50">
            ...
            <span className="text-xs text-slate-400">데이터를 불러오는 중...</span>
          </div>
        ) : (
          <>
            <div className={cn(
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">{isEasyCheckActive ? '웹훅 연동 활성화됨' : '연동 정보 없음'}</p>
                </div>
              </div>
              {isEasyCheckActive ? (
                <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200">연결됨</Badge>
              ) : (
                <Badge variant="outline" className="text-slate-400">미연결</Badge>
              )}
            </div>
              ...
              isTossActive ? "bg-blue-50/30 border-blue-200" : "bg-slate-50/30 border-slate-200 opacity-60"
               ...
               <div className="flex items-center gap-3">
                 ...
                 <div>
                   ...
                   <p className="text-[10px] text-slate-400">{isTossActive ? '연동 활성화됨' : '연동 정보 없음'}</p>
                 </div>
               </div>
               {isTossActive ? (
                 <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200">연결됨</Badge>
