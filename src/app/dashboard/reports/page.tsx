...
export default function ReportsPage() {
  ...
  const stats = useMemo(() => {
    ...
    filteredOrders.forEach(o => {
      const method = o.payment?.method || "기타";
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20">
      <PageHeader 
        title="보고서 및 통계" 
        description="기간별 매출, 지출 및 수익 현황을 한눈에 파악하세요."
        icon={BarChart3}
      >
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
         <Tabs value={dateRange} onValueChange={setDateRange} className="w-full md:w-auto">
            <TabsList className="bg-gray-100/80 p-1 rounded-2xl border border-white shadow-sm overflow-hidden h-11">
               <TabsTrigger value="today" className="rounded-xl font-bold px-6 h-9 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm">일일 정산</TabsTrigger>
               <TabsTrigger value="week" className="rounded-xl font-bold px-6 h-9 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm">주간</TabsTrigger>
               <TabsTrigger value="month" className="rounded-xl font-bold px-6 h-9 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm">월간</TabsTrigger>
               <TabsTrigger value="year" className="rounded-xl font-bold px-6 h-9 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm">연간</TabsTrigger>
            </TabsList>
         </Tabs>
         <Badge variant="outline" className="rounded-full px-4 py-1.5 font-bold text-gray-500 bg-white border-gray-100 shadow-sm">
            기준 기간: {dateRange === 'today' ? format(new Date(), 'yyyy-MM-dd') : 
                        dateRange === 'week' ? '최근 7일' : 
                        dateRange === 'month' ? format(new Date(), 'yyyy-MM') : 
                        format(new Date(), 'yyyy년')}
         </Badge>
      </div>
        ...
        <Card className="border-none shadow-2xl bg-gradient-to-br from-rose-500 to-rose-600 text-white overflow-hidden relative group">
           ...
           <CardHeader className="pb-2">
             <CardTitle className="text-rose-100 text-sm font-bold tracking-widest uppercase items-center flex gap-2">
                <TrendingDown className="w-4 h-4" /> 총 지출액
             </CardTitle>
           </CardHeader>
           <CardContent>
              {loading ? <Skeleton className="h-10 w-32 bg-white/20" /> : (
        ...
        <Card className={`border-none shadow-2xl overflow-hidden relative group transition-all ${
           ...
           <CardHeader className="pb-2">
             <CardTitle className="text-emerald-100 text-sm font-bold tracking-widest uppercase items-center flex gap-2">
                <DollarSign className="w-4 h-4" /> 순 이익
             </CardTitle>
           </CardHeader>
           <CardContent>
              {loading ? <Skeleton className="h-10 w-32 bg-white/20" /> : (
              ...
              <div className={`text-xs mt-4 font-bold ${stats.netProfit >= 0 ? 'text-emerald-100' : 'text-gray-400'} flex items-center gap-1`}>
                 매출 - 지출 손익계산 결과 {stats.netProfit < 0 && <AlertCircle className="w-3.5 h-3.5" />}
              </div>
           </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="border-none shadow-xl bg-white rounded-3xl overflow-hidden">
          <CardHeader className="bg-gray-50/50 border-b border-gray-100">
            <CardTitle className="text-lg font-bold text-gray-800 flex items-center gap-2">
               <Activity className="w-5 h-5 text-blue-500" /> 주요 성과 지표 (KPI)
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-6 space-y-6">
             <div className="flex justify-between items-center group">
                <div className="flex flex-col gap-0.5">
                   <span className="text-sm font-bold text-gray-500 group-hover:text-gray-900 transition-colors">평균 주문 단가</span>
                   <span className="text-xs text-muted-foreground">건당 발생하는 평균 매출액</span>
                </div>
             ...
             <div className="flex justify-between items-center group">
                <div className="flex flex-col gap-0.5">
                   <span className="text-sm font-bold text-gray-500 group-hover:text-gray-900 transition-colors">전체 주문 건수</span>
                   <span className="text-xs text-muted-foreground">선택 기간 내 유효 주문 총합</span>
                </div>
                <div className="text-right">
                   <div className="text-xl font-extrabold text-gray-900">{stats.orderCount.toLocaleString()} <span className="text-sm font-medium">건</span></div>
                   <div className="text-[10px] uppercase font-bold text-muted-foreground mt-0.5">Orders Total</div>
                </div>
             </div>

             <div className="pt-4 border-t border-gray-50">
                <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">결제 수단별 분포</h4>
                <div className="space-y-4">
                  {stats.paymentMethods.length > 0 ? stats.paymentMethods.map(([method, amount], idx) => {
                    ...
                    <div className="text-center py-4 text-gray-400 text-xs italic font-medium">데이터가 없습니다.</div>
                  )}
                </div>
             </div>
          </CardContent>
        ...
        <Card className="border-none shadow-xl bg-white rounded-3xl overflow-hidden">
          <CardHeader className="bg-gray-50/50 border-b border-gray-100">
            <CardTitle className="text-lg font-bold text-gray-800 flex items-center gap-2">
               <ShoppingBag className="w-5 h-5 text-amber-500" /> 최고 매출 상품 Top 5
            </CardTitle>
          </CardHeader>
             ...
             <div className="space-y-2">
                  ...
                  <div key={p.name} className="flex items-center gap-4 p-4 rounded-2xl bg-gray-50/30 hover:bg-gray-100/50 transition-all border border-transparent hover:border-gray-200 group">
                     ...
                     <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground font-medium flex items-center gap-2 mt-0.5">
                           전체 {p.count}개 판매 <span className="opacity-20">|</span> ₩{Math.round(p.total/p.count).toLocaleString()} 단가
                        </div>
                     </div>
                     <div className="text-right shrink-0">
                        ...
                        <Badge variant="secondary" className="text-[10px] font-bold h-5 px-1.5 bg-gray-200/50 text-gray-500 border-none">
                           {(stats.totalRevenue > 0 ? (p.total / stats.totalRevenue) * 100 : 0).toFixed(1)}% 점유
                        </Badge>
                     </div>
                  </div>
                ))}
                {stats.topProducts.length === 0 && (
                   <div className="h-64 flex flex-col items-center justify-center text-gray-300 space-y-2">
                      ...
                      <p className="font-bold">데이터가 수집되지 않았습니다.</p>
                   </div>
                )}
             </div>
          </CardContent>
        ...
        <Card className="border-none shadow-xl bg-white rounded-3xl overflow-hidden">
          <CardHeader className="bg-slate-900 text-white">
            <CardTitle className="text-lg font-bold flex items-center gap-2">
              <Wallet className="w-5 h-5 text-emerald-400" /> 금일 정산 상세 내역
            </CardTitle>
          </CardHeader>
             ...
             <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                   ...
                   <tbody className="divide-y divide-gray-100">
                      <tr>
                         <td className="px-6 py-4 font-bold text-gray-700">판매 매출 (카드)</td>
                         ...
                         <td className="px-6 py-4 text-xs text-gray-400">PG 결제 및 앱 내 결제 포함</td>
                      </tr>
                      <tr>
                         <td className="px-6 py-4 font-bold text-gray-700">판매 매출 (기타/현금)</td>
                         <td className="px-6 py-4 text-center">{orders.filter(o => isToday(new Date(o.order_date)) && o.payment?.method !== 'card' && o.payment?.method !== 'mainpay').length}</td>
                         <td className="px-6 py-4 text-right font-black text-indigo-600">
                            ₩{orders.filter(o => isToday(new Date(o.order_date)) && o.payment?.method !== 'card' && o.payment?.method !== 'mainpay').reduce((sum, o) => sum + (o.summary?.total || 0), 0).toLocaleString()}
                         </td>
                         <td className="px-6 py-4 text-xs text-gray-400">계좌이체, 현금, 기타 수단</td>
                      </tr>
                      <tr className="bg-rose-50/30">
                         <td className="px-6 py-4 font-bold text-rose-700">총 지출 (운영/배송)</td>
                         ...
                         <td className="px-6 py-4 text-xs text-rose-400">배송 시 자동 집계 포함</td>
                      </tr>
                      <tr className="bg-slate-900 text-white font-black text-lg">
                         <td className="px-6 py-6" colSpan={2}>금일 예치 손익계산 (Net)</td>
                         ...
                         <td className="px-6 py-6 text-xs font-normal text-slate-400">마감 시각: {format(new Date(), 'HH:mm')}</td>
                      </tr>
                   </tbody>
                </table>
             </div>
      ...
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 p-8 rounded-[40px] shadow-2xl relative overflow-hidden group">
         ...
         <div className="relative flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
            <div>
               <h3 className="text-3xl font-black text-white tracking-tight">수익 구조의 유료 멤버십 혜택</h3>
               <p className="text-gray-400 mt-2 text-lg font-medium">더 상세한 고객 분석과 기간별 성장 리포트로 비즈니스를 확장하세요.</p>
            </div>
            <Button className="bg-primary hover:bg-primary/90 text-white font-black px-10 h-14 rounded-2xl text-lg shadow-xl shadow-primary/30 active:scale-95 transition-all">
               프로 플랜 업그레이드 <ChevronRight className="w-5 h-5 ml-1" />
            </Button>
         </div>
      </div>
    </div>
