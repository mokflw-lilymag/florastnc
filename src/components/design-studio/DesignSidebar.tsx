      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-4">
        ...
        <div className="mb-5">
          ...
          >
            ...
            <span className="text-[12px] font-black text-slate-700">{D.magicStudio}</span>
          </button>
        </div>
        ...
        <div className="mb-6">
          ...
          >
            ...
            <div className="text-left">
              <span className="block leading-tight">{D.templateVault}</span>
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{D.templateVaultSub}</span>
            </div>
          </button>
          ...
          {expandedSections.includes('format') && (
            <div className="px-6 pb-6 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="space-y-2">
                ...
                >
                  <optgroup label={D.optgroupCard}>
                    {PAPER_PRESETS.filter(p => !p.id.startsWith('formtec-')).map(p => (
                      <option key={p.id} value={p.id}>{paperPresetLabel(p.id, p.label)}</option>
                    ))}
                  </optgroup>
                  <optgroup label={D.optgroupFormtec}>
                    {PAPER_PRESETS.filter(p => p.id.startsWith('formtec-')).map(p => (
                      <option key={p.id} value={p.id}>{paperPresetLabel(p.id, p.label)}</option>
                    ))}
                  </optgroup>
              ...
              {foldType === 'half' && (
                <div className="flex items-center justify-between p-3 bg-indigo-50/50 rounded-2xl border border-indigo-100/50">
                  <span className="text-[11px] font-black text-indigo-700">{D.foldGuideline}</span>
                  <button
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          >
            ...
            <div className="text-left">
              <p className="text-sm font-black text-slate-800">{D.suggestedMessages}</p>
              <p className="text-[10px] text-slate-400 font-bold tracking-tight">{D.suggestedMessagesSub}</p>
            </div>
          </button>
          ...
          >
            ...
            <div className="text-left">
              <p className="text-sm font-black text-slate-800">{D.quotesLib}</p>
              <p className="text-[10px] text-slate-400 font-bold tracking-tight">{D.quotesSub}</p>
            </div>
          </button>
        </div>
          ...
          <div className="border border-slate-100 rounded-[2rem] overflow-hidden bg-white shadow-sm">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-black text-slate-700">{D.autoRecipient}</span>
              </div>
                ...
                <div className="p-5 bg-slate-50 rounded-[2rem] border border-slate-100 space-y-4">
                  <div className="space-y-2">
                    <div className="flex justify-between items-center px-1">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{D.toLabel}</span>
                    ...
                    {showToField && (
                      <input
                        ...
                        placeholder={D.toPlaceholder} className="w-full p-3 bg-white border border-slate-200 rounded-xl text-xs font-bold focus:ring-2 focus:ring-indigo-100 outline-none"
                      />
                    )}
                  </div>
                  <div className="space-y-2 pt-2 border-t border-slate-200/50">
                    <div className="flex justify-between items-center px-1">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{D.fromLabel}</span>
                    ...
                    {showFromField && (
                      <input
                        ...
                        placeholder={D.fromPlaceholder} className="w-full p-3 bg-white border border-slate-200 rounded-xl text-xs font-bold focus:ring-2 focus:ring-indigo-100 outline-none"
                      />
                    )}
                  </div>
                </div>
              ...
              {!selectedBlock ? (
                ...
                <div className="space-y-6">
                  {isTextBlock ? (
                    <>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-indigo-500 uppercase tracking-widest ml-1">{D.propContent}</label>
                        <textarea
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{D.propFontSize}</label>
                          <input
                        ...
                        <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{D.propTextColor}</label>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-2">
                          <div className="flex justify-between items-center px-1">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{D.propRotation}</label>
                        ...
                        <div className="space-y-2">
                           <div className="flex justify-between items-center px-1">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{D.propOpacity}</label>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{D.propLineHeight}</label>
                        ...
                        <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{D.propLetterSpacing}</label>
                          <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl">
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{D.propFontFamily}</label>
                        <select
                        ...
                        >
                          <optgroup label={D.fontGroupBold}>
                            <option value="'GmarketSansBold', sans-serif">{DS.studioFont_gmarket}</option>
                            <option value="'Bagel Fat One', cursive">{DS.studioFont_bagel}</option>
                            <option value="'Black Han Sans', sans-serif">{DS.studioFont_blackHan}</option>
                            <option value="'Do Hyeon', sans-serif">{DS.studioFont_doHyeon}</option>
                            <option value="'Jua', sans-serif">{DS.studioFont_jua}</option>
                          </optgroup>
                          <optgroup label={D.fontGroupElegant}>
                            <option value="'JoseonPalace', serif">{DS.studioFont_joseonPalace}</option>
                            <option value="'Gowun Batang', serif">{DS.studioFont_gowunBatang}</option>
                            <option value="'Nanum Myeongjo', serif">{DS.studioFont_nanumMyeongjo}</option>
                            <option value="'Song Myung', serif">{DS.studioFont_songMyung}</option>
                            <option value="'Yeon Sung', serif">{DS.studioFont_yeonSung}</option>
                          </optgroup>
                          <optgroup label={D.fontGroupArtistic}>
                            <option value="'Nanum Pen Script', cursive">{DS.studioFont_nanumPen}</option>
                            <option value="'Gaegu', cursive">{DS.studioFont_gaegu}</option>
                            <option value="'Gamja Flower', cursive">{DS.studioFont_gamjaFlower}</option>
                            <option value="'Single Day', cursive">{DS.studioFont_singleDay}</option>
                            <option value="'East Sea Dokdo', cursive">{DS.studioFont_eastSeaDokdo}</option>
                            <option value="'Poor Story', system-ui">{DS.studioFont_poorStory}</option>
                          </optgroup>
                          <optgroup label={D.fontGroupCommon}>
                            <option value="'Noto Sans KR', sans-serif">{DS.studioFont_notoSansKr}</option>
                            <option value="'Dongle', sans-serif">{DS.studioFont_dongle}</option>
                            <option value="'Gugi', cursive">{DS.studioFont_gugi}</option>
                          </optgroup>
                        </select>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 pb-4 border-b border-slate-100">
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{D.propStroke}</label>
                          <input
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{D.propColor}</label>
        ...
        <div className="border border-slate-100 rounded-[2rem] overflow-hidden bg-white shadow-sm">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-black text-slate-700">{D.shopBranding}</span>
            </div>
          ...
          {expandedSections.includes('branding') && (
            <div className="px-6 pb-6 space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="p-5 bg-slate-50/50 rounded-[2rem] border border-slate-100/50 flex flex-col items-center gap-4 text-center">
                ...
                <div>
                  <p className="text-sm font-black text-slate-800">{shopSettings.name || D.noShopInfo}</p>
                  <button onClick={onOpenShopSettings} className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mt-1 hover:underline">{D.editProfile}</button>
                </div>
              </div>

              <div className="space-y-2 px-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">{D.includeItems}</label>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { id: 'logo', label: D.brandingLogo, icon: <ImageIcon size={12} /> },
                    { id: 'name', label: D.brandingName, icon: <Type size={12} /> },
                    { id: 'tel', label: D.brandingTel, icon: <Phone size={12} /> },
                    { id: 'website', label: D.brandingWeb, icon: <Globe size={12} /> }
                  ].map((opt) => (
                    <button
                      key={opt.id}
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                ...
                >
                  <span>{D.placeFront}</span>
                  <span className="text-[8px] font-normal opacity-50">{D.coverTag}</span>
                </button>
                <button
                ...
                >
                  <span>{D.placeBack}</span>
                  <span className="text-[8px] font-normal opacity-50">{D.backTag}</span>
                </button>
              </div>
            </div>
          )}
